...
- `src/email/` — `EmailModule` with `EmailService.sendMeetingInvitation` (subject/body built in `invitation.template.ts`); SMTP transport from `mailTransportProvider` (`nodemailer`, no-op when `SMTP_HOST` unset) and sender from `mailFromProvider`, both via `ConfigService`
