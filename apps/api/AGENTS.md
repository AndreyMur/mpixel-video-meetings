...
- `src/meetings/` — `MeetingsModule` with `POST /meetings`, `GET /meetings`, `GET /meetings/:id`, `PATCH /meetings/:id` (partial update of own meeting), `DELETE /meetings/:id` (own meeting only; 409 `Conflict` with a clear message when the meeting has files, nothing is deleted), `POST /meetings/:id/invitations` (only organizer; `{ email }` must be a participant and not the organizer's email, 400 otherwise; 403 for non-organizer, 404 when the meeting is missing; sends a single invitation email with the meeting link and does not fail when SMTP errors); all protected by `JwtAuthGuard`; uses CQRS (`CreateMeetingCommand`, `GetMeetingsQuery`, `GetMeetingQuery`, `UpdateMeetingCommand`, `DeleteMeetingCommand`, `SendInvitationCommand`), `CreateMeetingDto` (`title`, `description?`, `date`, `participants?` — each validated as an email), `UpdateMeetingDto` (all of `title`/`description`/`date`/`participants` optional, emails validated), `SendInvitationDto` (`email`), Prisma `Meeting` model scoped to the current user; the creator's email is always stored in `participants` (deduplicated, minimum one participant) on create and update; sends email invitations via `MeetingInvitationService` (fans out to every participant except the organizer through `EmailService` on create and on any actual update of title/description/date/participants, failures are logged and don't fail the request)
