...
- `src/meetings/` — `MeetingsModule` with `POST /meetings`, `GET /meetings`, `GET /meetings/:id`, `PATCH /meetings/:id` (partial update of `title`/`date`/`participants`, ownership check, 404 for missing); all protected by `JwtAuthGuard`; uses CQRS (`CreateMeetingCommand`, `UpdateMeetingCommand`, `GetMeetingsQuery`, `GetMeetingQuery`), `CreateMeetingDto` (`title`, `date`, `participants`), `UpdateMeetingDto` (all fields optional), Prisma `Meeting` model scoped to the current user; sends email invitations via `MeetingInvitationService` (fans out to every participant through `EmailService`, failures are logged and don't fail the request)
